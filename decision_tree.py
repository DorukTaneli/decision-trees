...
def pretty_evaluate (confusion_matrix, trees):
	...
	precision, recall = get_precision_recall (confusion_matrix,trees)
	...
	for i in range(4):
		print('Predicted class (%i):\t'%i, round(confusion_matrix[i][0],0),'\t', round(confusion_matrix[i][1],0),'\t',round(confusion_matrix[i][2],0),'\t', round(confusion_matrix[i][3],0))
	print()
	...
	print()
	print('Depth of the final tree:', find_depth(trees))
...
visualize_tree(clean_trees[1], 10)
visualize_tree(clean_trees_prune[1], 10)

#NOISY DATA

noisy_trees, noisy_trees_prune, noisy_average_matrix, noisy_average_matrix_prune = cross_validation(10, noisy, True)

visualize_tree(noisy_trees[1], 10)
visualize_tree(noisy_trees_prune[1], 10)
